...
export class WidgetTable extends Component {
    render() {
        ...
        const Row = ({label, value, isHeader}) => (
            <div className={`${!isHeader ? 'border-top' : ''} row widget-table-row pt-2 pb-2 mr-4 ml-4`}>
                <div className={`label-col col-8 p-0 ${isHeader ? 'font-weight-bold' : ''}`}>{label}</div>
                <div className={`value-col col-4 text-right pl-0 ${isHeader ? 'font-weight-bold' : ''}`}>{!isHeader ? value: 'Count'}</div>
            </div>
        )
